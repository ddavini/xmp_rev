...
install_packages() {
    case "$PKG_MANAGER" in
        apt)
            ...
            ;;
        dnf)
            sudo dnf install -y gcc-c++ make pkgconfig SDL2-devel zlib-devel
            ;;
        pacman)
            sudo pacman -Sy --needed --noconfirm base-devel pkgconf sdl2 zlib
            ;;
        *)
            echo "error: could not detect a supported package manager (apt/dnf/pacman)." >&2
            echo "Please install a C++20 compiler, make, pkg-config, and the SDL2 and" >&2
            echo "zlib development headers manually, then re-run this script." >&2
            exit 1
            ;;
    esac
}
...
# zlib: decompresses zipped/gzipped tracker modules (.mdz/.xmz/.s3z).
pkg-config --exists zlib 2>/dev/null || missing=1
